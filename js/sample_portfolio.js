...
$(document).ready(function () {
  ...
  // ✅ about-section CV 블록 순차 등장 애니메이션
  function revealCvBlocks() {
    var $aboutSection = $(".about-section");
    var $blocks = $aboutSection.find(".cv-block");
    var windowH = $(window).height();
    var sectionTop = $aboutSection.offset().top;
    var scrollY = $(window).scrollTop();
    if (scrollY + windowH > sectionTop + 100) {
      $blocks.each(function (i) {
        var $block = $(this);
        setTimeout(function () {
          $block.addClass("cv-revealed");
        }, i * 600); // 딜레이를 600ms로 증가
      });
    } else {
      $blocks.removeClass("cv-revealed");
    }
  }

  $(window).on("scroll resize", revealCvBlocks);
  revealCvBlocks();
});
